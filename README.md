...
2. Setup your Coinbase Pro account portfolios (profiles), this is an important part of the bot. The bot will swoop up any available balance in a profile and start trading with it, so in order to safely store some of the profits you must specify another profile to deposit to. Currently, Coinbase Pro limits profiles to five. Create a new portfolio for each bot you want to run, for example you could create one called BTC trader that the bot trades BTC inside of. Then specify the name of the trading bot in the program and the name of profile to deposit any profits too if you plan to save some or all of the profits. The default sets the deposit profile to "default" (Default Portfolio) if you want the profits to go into a different profile you can specify the name of it in there instead. Don't trade in the default profile because if you transfer money in it then it could get swept up by the bot before you can allocate it where you want to.
